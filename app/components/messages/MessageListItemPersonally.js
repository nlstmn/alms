...
function MessageListItemPersonally({ data, navigation, refresh, currentUserId }) {
    ...
    return (
        <TouchableOpacity style={style.item} activeOpacity={0.5} onPress={() => {
            ...
            <Avatar size={40} rounded title={data.recipientName?.split(' ').slice(0, 3).map(function (item) { return item[0] }).join('').toUpperCase()} />
            <View style={style.content}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <TextView style={style.title} weight="bold">{data.recipientName.trim()}</TextView>
...
export default MessageListItemPersonally
